...
slova = [slovo.strip() for slovo in file.read().split(', ')]
file.close()
...
height = AMOGUS_SIZE+(max_hornich*ACCESORY_HEIGHT) + \
    (max_dolnich*ACCESORY_HEIGHT)
...
for i, komponenta in enumerate(flipped_components):
    platno_amongus = Image.new(
        "RGBA", (AMOGUS_SIZE, height), (255, 255, 255, 255))
    upper_accesories = 0
    lower_accesories = 0
    to_flip_up = False
    to_flip_side = False
    ...
    if not is_body:
        for body in bodies:
            if body in komponenta:
                if without_eyes:
                    pass  # TODO
                platno_amongus.paste(
                    images_dict[body], (0, ACCESORY_HEIGHT*max_hornich), images_dict[body])
                is_body = True
                amongus_body = body

    if not is_body:
        platno_amongus.paste(
            images_dict["amogus"], (0, ACCESORY_HEIGHT*max_hornich), images_dict["amogus"])
        amongus_body = "amogus"
    ...
    for item in komponenta:
        if item in upper:
            platno_amongus.paste(images_dict[item], (0, platno_amongus.height-ACCESORY_HEIGHT*max_dolnich-AMOGUS_SIZE -
                                 ACCESORY_HEIGHT-ACCESORY_HEIGHT*(upper_accesories-upper_index)+ACCESORY_HEIGHT), images_dict[item])
            ...
            if item == "F":
                platno_amongus.paste(
                    images_dict[item], (0, ACCESORY_HEIGHT*max_hornich+ACCESORY_HEIGHT+20), images_dict[item])
            else:
                platno_amongus.paste(
                    images_dict[item], (0, ACCESORY_HEIGHT*max_hornich+ACCESORY_HEIGHT+10), images_dict[item])
        elif item in lower:
            if item == "L":
                # L jako lyze mohou byt vyse
                spec_height = (ACCESORY_HEIGHT*max_hornich +
                               AMOGUS_SIZE+ACCESORY_HEIGHT*lower_index)-14
            elif item == "M" or "U":
                spec_height = (ACCESORY_HEIGHT*max_hornich +
                               AMOGUS_SIZE+ACCESORY_HEIGHT*lower_index)-16
            else:
                # nestane se
                spec_height = ACCESORY_HEIGHT*max_hornich + \
                    AMOGUS_SIZE+ACCESORY_HEIGHT*lower_index
            platno_amongus.paste(
                images_dict[item], (0, spec_height), images_dict[item])
            lower_index += 1
        elif item in oci:
            if amongus_body == "O":
                platno_amongus.paste(
                    images_dict[item], (12, ACCESORY_HEIGHT*max_hornich+19), images_dict[item])
            elif amongus_body == "A":
                platno_amongus.paste(
                    images_dict[item], (0, ACCESORY_HEIGHT*max_hornich+10), images_dict[item])
            elif amongus_body == "E":
                platno_amongus.paste(
                    images_dict[item], (3, ACCESORY_HEIGHT*max_hornich+14), images_dict[item])
            else:
                platno_amongus.paste(
                    images_dict[item], (3, ACCESORY_HEIGHT*max_hornich+14), images_dict[item])
    if to_flip_side:
        region = platno_amongus.crop(
            (0, 0, platno_amongus.width, platno_amongus.height))
    ...
    if to_resize:
        ...
        region = region.resize(
            (platno_amongus.width//2, platno_amongus.height//2))
        platno_amongus.paste(
            (255, 255, 255), (0, 0, platno_amongus.width, platno_amongus.height))  # vymazat pozadi
        platno_amongus.paste(
            region, (platno_amongus.width//4, platno_amongus.height//4), (region))
